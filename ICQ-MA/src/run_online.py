...
class replaybuffer_mine:
    ...
    def add_online_batch(self,batch):# sample trajectories not tuples
        ...
        if self.size == 0:
            self.actions_h = batch.data.transition_data['actions']
            self.actions_onehot_h = batch.data.transition_data['actions_onehot']
            self.avail_actions_h = batch.data.transition_data['avail_actions']
            self.filled_h = batch.data.transition_data['filled']
            self.obs_h = batch.data.transition_data['obs']
            self.reward_h = batch.data.transition_data['reward']
            self.state_h = batch.data.transition_data['state']
            self.terminated_h = batch.data.transition_data['terminated']
        else:
