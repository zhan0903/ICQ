...
class ReplayBuffer(EpisodeBatch):
    ...
    def insert_episode_batch(self, ep_batch, batch_size, max_seq_length):
        if True:
            self.update(ep_batch,
                        slice(self.buffer_index, self.buffer_index + batch_size),
                        slice(0, max_seq_length),
                        mark_filled=False)
            self.buffer_index = (self.buffer_index + batch_size)
